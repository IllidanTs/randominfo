...
def get_address():
    ...
    try:
        with open(full_path('data.csv'), 'r') as csvfile:
            ...
            next(addrFile)  # Пропустити заголовок
            for i in range(5, 12):
                allAddrs = []
                ...
                if not allAddrs:
                    allAddrs.append("N/A")  # Додати значення за замовчуванням, якщо колонка порожня
                full_addr.append(choice(allAddrs))
        full_addr = dict(zip(addrParam, full_addr))
    except Exception as e:
        print(f"Error reading address data: {e}")
        full_addr = dict(zip(addrParam, ["N/A"] * len(addrParam)))
    return full_addr
